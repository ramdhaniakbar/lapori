...
<div class="w-full h-[267px] bg-black-color relative mb-[112px]"
   ...
   <div class="flex justify-center items-center text-center h-full">
      <div class="flex flex-col">
         ...
         <div class="text-[#858690] font-bold text-[48px] inline-block">{{ $count_report }}</div>
      </div>
   </div>
</div>
...
<div class="container-padded-home mx-auto">
   ...
   <div class="flex px-6 py-6 space-x-14 overflow-x-auto mb-[112px]">
      ...
      <div class="w-[404px] flex-shrink-0 card-shadow rounded-lg py-[26px] px-[24px] bg-white relative"
         ...
         <a href="{{ route('lapor.show', $success_report->id) }}">
            <h4 class="text-[20px] text-black-color font-semibold mb-2.5">{{ $success_report->title_report }}</h4>
         </a>
         <p class="text-[13px] font-medium text-paragraph-color mb-2.5">{{ $success_report->body_report }}</p>
         <div class="flex items-center space-x-2.5">
            <img src="{{ asset('assets/frontsite/images/user-profile.svg') }}" class="w-[33px] rounded-full"
               alt="user_1">
            <span class="text-[13px] font-medium text-black-color">{{ $success_report->user->name }}</span>
         </div>
      </div>
      @empty
      <p>Belum ada data!</p>
   ...
   <div class="flex flex-col px-6 py-6 space-y-8 overflow-y-auto h-156 mb-[128px]">
      ...
      <div class="w-full card-shadow rounded-lg p-[42px]">
         <h4 class="text-[20px] text-black-color font-semibold mb-4">{{ $latest_report->title_report }}</h4>
         <p class="text-[13px] font-medium text-paragraph-color mb-4">{{ $latest_report->body_report }}</p>
         <div class="flex justify-between items-center">
            <div class="flex items-center space-x-2.5">
               <img src="{{ asset('assets/frontsite/images/user-profile.svg') }}" class="w-[33px] rounded-full"
                  alt="user_1">
               <span class="text-[13px] font-medium text-black-color">{{ $latest_report->user->name }}</span>
            </div>
            <div class="flex space-x-5">
               <span class="text-[12px] text-black-color">{{ $latest_report->created_at->diffForHumans() }}</span>
            </div>
         </div>
      </div>
      @empty
